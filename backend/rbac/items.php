<?php
return [
    'admin' => [
        'type' => 1,
        'description' => 'Full administration usage',
        'children' => [
            '/*',
            '/admin/*',
            '/sms/*',
            '/site/*',
            '/site/error',
            '/site/index',
            '/site/login',
            '/site/logout',
            '/sms/send',
            '/sms/status',
            '/admin/assignment/index',
            '/admin/assignment/view',
            '/admin/assignment/assign',
            '/admin/assignment/search',
            '/admin/assignment/*',
            '/admin/default/index',
            '/admin/default/*',
            '/admin/menu/index',
            '/admin/menu/view',
            '/admin/menu/create',
            '/admin/menu/update',
            '/admin/menu/delete',
            '/admin/menu/*',
            '/admin/permission/index',
            '/admin/permission/view',
            '/admin/permission/create',
            '/admin/permission/update',
            '/admin/permission/delete',
            '/admin/permission/assign',
            '/admin/permission/search',
            '/admin/permission/*',
            '/admin/role/index',
            '/admin/role/view',
            '/admin/role/create',
            '/admin/role/update',
            '/admin/role/delete',
            '/admin/role/assign',
            '/admin/role/search',
            '/admin/role/*',
            '/admin/route/index',
            '/admin/route/create',
            '/admin/route/assign',
            '/admin/route/search',
            '/admin/route/*',
            '/admin/rule/index',
            '/admin/rule/view',
            '/admin/rule/create',
            '/admin/rule/update',
            '/admin/rule/delete',
            '/admin/rule/*',
            '/debug/*',
            '/gii/*',
            '/appartments/*',
            '/site/test',
            '/appartments/index',
            '/appartments/view',
            '/appartments/create',
            '/appartments/update',
            '/appartments/delete',
            'UserCRUD',
            '/debug/default/index',
            '/debug/default/view',
            '/debug/default/toolbar',
            '/debug/default/download-mail',
            '/debug/default/*',
            '/gii/default/index',
            '/gii/default/view',
            '/gii/default/preview',
            '/gii/default/diff',
            '/gii/default/action',
            '/gii/default/*',
            'Worker',
            'UserAccess',
            'AdminSettings',
            'SendSMSMessage',
            '/sendsms/index',
            '/sendsms/view',
            '/sendsms/create',
            '/sendsms/update',
            '/sendsms/delete',
            '/sendsms/*',
            '/site/signup',
            '/sms-template/index',
            '/sms-template/view',
            '/sms-template/create',
            '/sms-template/update',
            '/sms-template/delete',
            '/sms-template/*',
            '/sendsms/templatetext',
            '/sendsms/templist',
        ],
    ],
    '/admin/*' => [
        'type' => 2,
    ],
    '/*' => [
        'type' => 2,
    ],
    '/sms/*' => [
        'type' => 2,
    ],
    '/site/*' => [
        'type' => 2,
    ],
    '/site/error' => [
        'type' => 2,
    ],
    '/site/index' => [
        'type' => 2,
    ],
    '/site/login' => [
        'type' => 2,
    ],
    '/site/logout' => [
        'type' => 2,
    ],
    '/sms/send' => [
        'type' => 2,
    ],
    '/sms/status' => [
        'type' => 2,
    ],
    '/admin/assignment/index' => [
        'type' => 2,
    ],
    '/admin/assignment/view' => [
        'type' => 2,
    ],
    '/admin/assignment/assign' => [
        'type' => 2,
    ],
    '/admin/assignment/search' => [
        'type' => 2,
    ],
    '/admin/assignment/*' => [
        'type' => 2,
    ],
    '/admin/default/index' => [
        'type' => 2,
    ],
    '/admin/default/*' => [
        'type' => 2,
    ],
    '/admin/menu/index' => [
        'type' => 2,
    ],
    '/admin/menu/view' => [
        'type' => 2,
    ],
    '/admin/menu/create' => [
        'type' => 2,
    ],
    '/admin/menu/update' => [
        'type' => 2,
    ],
    '/admin/menu/delete' => [
        'type' => 2,
    ],
    '/admin/menu/*' => [
        'type' => 2,
    ],
    '/admin/permission/index' => [
        'type' => 2,
    ],
    '/admin/permission/view' => [
        'type' => 2,
    ],
    '/admin/permission/create' => [
        'type' => 2,
    ],
    '/admin/permission/update' => [
        'type' => 2,
    ],
    '/admin/permission/delete' => [
        'type' => 2,
    ],
    '/admin/permission/assign' => [
        'type' => 2,
    ],
    '/admin/permission/search' => [
        'type' => 2,
    ],
    '/admin/permission/*' => [
        'type' => 2,
    ],
    '/admin/role/index' => [
        'type' => 2,
    ],
    '/admin/role/view' => [
        'type' => 2,
    ],
    '/admin/role/create' => [
        'type' => 2,
    ],
    '/admin/role/update' => [
        'type' => 2,
    ],
    '/admin/role/delete' => [
        'type' => 2,
    ],
    '/admin/role/assign' => [
        'type' => 2,
    ],
    '/admin/role/search' => [
        'type' => 2,
    ],
    '/admin/role/*' => [
        'type' => 2,
    ],
    '/admin/route/index' => [
        'type' => 2,
    ],
    '/admin/route/create' => [
        'type' => 2,
    ],
    '/admin/route/assign' => [
        'type' => 2,
    ],
    '/admin/route/search' => [
        'type' => 2,
    ],
    '/admin/route/*' => [
        'type' => 2,
    ],
    '/admin/rule/index' => [
        'type' => 2,
    ],
    '/admin/rule/view' => [
        'type' => 2,
    ],
    '/admin/rule/create' => [
        'type' => 2,
    ],
    '/admin/rule/update' => [
        'type' => 2,
    ],
    '/admin/rule/delete' => [
        'type' => 2,
    ],
    '/admin/rule/*' => [
        'type' => 2,
    ],
    '/debug/*' => [
        'type' => 2,
    ],
    '/gii/*' => [
        'type' => 2,
    ],
    '/appartments/*' => [
        'type' => 2,
    ],
    '/site/test' => [
        'type' => 2,
    ],
    '/appartments/index' => [
        'type' => 2,
    ],
    '/appartments/view' => [
        'type' => 2,
    ],
    '/appartments/create' => [
        'type' => 2,
    ],
    '/appartments/update' => [
        'type' => 2,
    ],
    '/appartments/delete' => [
        'type' => 2,
    ],
    'UserCRUD' => [
        'type' => 2,
    ],
    '/debug/default/index' => [
        'type' => 2,
    ],
    '/debug/default/view' => [
        'type' => 2,
    ],
    '/debug/default/toolbar' => [
        'type' => 2,
    ],
    '/debug/default/download-mail' => [
        'type' => 2,
    ],
    '/debug/default/*' => [
        'type' => 2,
    ],
    '/gii/default/index' => [
        'type' => 2,
    ],
    '/gii/default/view' => [
        'type' => 2,
    ],
    '/gii/default/preview' => [
        'type' => 2,
    ],
    '/gii/default/diff' => [
        'type' => 2,
    ],
    '/gii/default/action' => [
        'type' => 2,
    ],
    '/gii/default/*' => [
        'type' => 2,
    ],
    'Worker' => [
        'type' => 1,
        'description' => 'Default Worker',
        'children' => [
            'UserAccess',
            'SendSMSMessage',
            '/sendsms/templatetext',
            '/sendsms/templist',
        ],
    ],
    'UserAccess' => [
        'type' => 2,
        'description' => 'дает право на вход',
        'children' => [
            '/site/*',
            '/site/error',
            '/site/index',
            '/site/login',
            '/site/logout',
        ],
    ],
    'AdminSettings' => [
        'type' => 2,
        'description' => 'Меню для админки',
        'children' => [
            'UserCRUD',
            'UserAccess',
            '/admin/*',
            '/*',
            '/sms/*',
            'SendSMSMessage',
        ],
    ],
    '/sendsms/index' => [
        'type' => 2,
    ],
    '/sendsms/view' => [
        'type' => 2,
    ],
    '/sendsms/create' => [
        'type' => 2,
    ],
    '/sendsms/update' => [
        'type' => 2,
    ],
    '/sendsms/delete' => [
        'type' => 2,
    ],
    '/sendsms/*' => [
        'type' => 2,
    ],
    '/site/signup' => [
        'type' => 2,
    ],
    '/sms-template/index' => [
        'type' => 2,
    ],
    '/sms-template/view' => [
        'type' => 2,
    ],
    '/sms-template/create' => [
        'type' => 2,
    ],
    '/sms-template/update' => [
        'type' => 2,
    ],
    '/sms-template/delete' => [
        'type' => 2,
    ],
    '/sms-template/*' => [
        'type' => 2,
    ],
    'SendSMSMessage' => [
        'type' => 2,
        'description' => 'Дает возможность пользователям отправлять сообщения из админки',
        'children' => [
            '/sms/*',
            '/sms/send',
            '/sms/status',
            '/sendsms/index',
            '/sendsms/view',
            '/sendsms/create',
            '/sendsms/update',
        ],
    ],
    '/sendsms/templatetext' => [
        'type' => 2,
    ],
    '/sendsms/templist' => [
        'type' => 2,
    ],
];
